-- Total = sodex delta (source of truth from official LB).
-- No GREATEST — sodex is the base, not a fallback.
-- Spot = max(sodex_delta - futures_delta, 0).
-- Now that snapshot is correct (via 24h fix), raw delta is accurate.
