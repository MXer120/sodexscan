-- Fix: add total_pnl/spot_pnl sort options, include PnL-only accounts in filter
    ...
    SELECT json_agg(row_to_json(t)) INTO result FROM (
      SELECT * FROM (
        ...
        WHERE cw.week_number = actual_week
          ...
          AND (cw.cumulative_volume > 0 OR cw.sodex_total_volume > 0 OR cw.sodex_pnl != 0)
      ) sub
      WHERE
        CASE p_sort
          ...
          WHEN 'total_pnl' THEN sub.weekly_sodex_pnl != 0
          WHEN 'spot_pnl' THEN sub.weekly_spot_pnl != 0
          ELSE true
        END
      ORDER BY
        CASE p_sort
          ...
          WHEN 'total_pnl' THEN sub.weekly_sodex_pnl
          WHEN 'spot_pnl' THEN sub.weekly_spot_pnl
      ...
      SELECT * FROM (
        ...
        WHERE cw.week_number = actual_week
          ...
          AND ((cw.cumulative_volume - COALESCE(pw.cumulative_volume, 0)) > 0
               ...
               OR (cw.sodex_pnl - COALESCE(pw.sodex_pnl, 0)) != 0)
      ) sub
      WHERE
        CASE p_sort
          WHEN 'futures_volume' THEN sub.weekly_volume > 0
