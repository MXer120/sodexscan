...
// ── DS KPI Card ────────────────────────────────────────────────
function KpiCard({ label, value, colorClass = undefined, deltaValue = undefined, deltaDir = undefined, deltaLabel = undefined, spark = undefined }) {
...
function DsTable({ title, count = undefined, children, page = undefined, totalPages = undefined, onPage = undefined, pageSize = undefined, onPageSize = undefined, totalRows = undefined }) {
...
// ── Main component ─────────────────────────────────────────────
export default function ReportDashboard({ trades = [] as TradeRow[], meta = {} as TradeMeta, onNewReport, onRefresh }) {
  ...
  const pnlChartData = useMemo(() => {
    const byDay: Record<string, { date: string; dailyPnl: number }> = {}
  ...
  const breakdownRows = useMemo(() => {
    const buckets: Record<string, { label: string; trades: number; wins: number; grossPnl: number; fees: number }> = {}
    for (const t of trades) {
  ...
  const symbolRows = useMemo(() => {
    const m: Record<string, { symbol: string; trades: number; grossPnl: number; fees: number }> = {}
    for (const t of trades) {
