...
export interface ThemeSettings {
  ...
  bearishColor: string
  accentColor: string
  /** Optional hex override applied on top of the active scheme's primary. */
...
export const STORAGE_KEY = 'themeSettings.v1'
...
export function applyTheme(settings: ThemeSettings) {
  ...
  const { colorScheme, mode, bullishColor, bearishColor, accentColor, customPrimary } = settings
  const scheme = COLOR_SCHEMES[colorScheme] ?? COLOR_SCHEMES['cli']
  const colors = scheme[mode] ?? scheme['dark']
  const root = document.documentElement

  // Custom primary override — when the user picks an arbitrary color it replaces
  // the scheme's primary across the whole app (incl. spot/volume/accent).
  const useCustom = !!(customPrimary && isValidHex(customPrimary))
  const primary = useCustom ? customPrimary! : colors.primary
  const primaryDark = useCustom ? adjustLightness(primary, -10) : colors.primaryDark
  const primaryLight = useCustom ? adjustLightness(primary, 10) : colors.primaryLight
  const primaryRgb = useCustom ? hexToRgb(primary) : colors.primaryRgb
  ...
  // When user has a custom primary, route spot/volume/accent through it so the
  // picker affects the whole app (the user-facing intent of "primary color").
  const spot = useCustom ? primary : colors.spot
  const spotBg = useCustom ? `rgba(${primaryRgb}, 0.15)` : colors.spotBg
  const volume = useCustom ? primary : colors.volume

  root.style.setProperty('--color-spot', spot)
  root.style.setProperty('--color-spot-bg', spotBg)
  root.style.setProperty('--color-volume', volume)
  ...
  // Apply accent color (custom primary overrides scheme's accent setting)
  const accent = useCustom ? primary : accentColor
  root.style.setProperty('--color-accent', accent)
  root.style.setProperty('--color-accent-rgb', hexToRgb(accent))

  // Bridge to design-system tokens so /design-system shows the picked color too.
  root.style.setProperty('--ds-accent', primary)
  root.style.setProperty('--ds-accent-hover', primaryLight)
  root.style.setProperty('--ds-accent-dim', primaryDark)
  root.style.setProperty('--ds-accent-soft', `rgba(${primaryRgb}, 0.15)`)
  root.style.setProperty('--ds-pixel-high', primary)
  root.style.setProperty('--ds-pixel-low', primaryLight)
  ...
  const faviconDataUrl = makeThemeFaviconDataUrl(primary)
...
// Lighten/darken a hex color by adjusting HSL lightness by `delta` percentage points.
...
// Load saved settings from localStorage (returns DEFAULT_THEME on miss/parse-fail).
export function loadStoredSettings(): ThemeSettings {
