...
import FollowModal from './copy/FollowModal'
...
const CoinLogo = memo(({ symbol, size = '18px' }: { symbol: string; size?: string }) => {
  ...
  if (!logoUrl || !show) {
    return (
      <div style={{
        ...
        color: 'var(--muted-foreground)', border: '1px solid var(--border)', flexShrink: 0
      }}>
...
const CopyableAddress = memo(({ address, truncated = true, onCopy }: { address: string; truncated?: boolean; onCopy?: () => void }) => {
...
const TransactionBadge = memo(({ type, color, bgColor, icon }: { type: string; color: string; bgColor: string; icon: React.ReactNode }) => (
  <div style={{
...
const SideBadge = memo(({ side, leverage, BULLISH_COLOR, BEARISH_COLOR }: { side: string; leverage: any; BULLISH_COLOR: string; BEARISH_COLOR: string }) => {
...
const SortArrows = memo(({ active, dir }: { active: boolean; dir: string }) => (
...
export default function MainnetTracker({ walletAddress = null, accountId: propAccountId = null, searchBox = null, tagSection = null, onWalletChange = null }: { walletAddress?: any; accountId?: any; searchBox?: any; tagSection?: any; onWalletChange?: any }) {
  ...
  const statsSummary = useMemo(() => {
    ...
    let rating = 'Neutral'
    let sentimentColor = 'var(--muted-foreground)'
  ...
  const tradeSymbolOptions = useMemo(() => {
    ...
    positionHistory.forEach(p => {
      const sym = symbolMap[p.symbol_id]
      if (sym) set.add(getBaseCoin(sym))
    })
    return Array.from(set).sort()
  }, [positionHistory, symbolMap])

  const sortedTrades = useMemo(() => {
    const filtered = tradeSymbolFilter
      ? positionHistory.filter(p => {
          const sym = symbolMap[p.symbol_id] || ''
          return getBaseCoin(sym).toUpperCase() === tradeSymbolFilter.toUpperCase()
        })
      : positionHistory
    return [...filtered]
      .sort((a, b) => {
      ...
      .slice(0, tradesLimit)
  }, [positionHistory, tradeSortField, tradeSortDir, tradesLimit, symbolMap, tradeSymbolFilter])
  ...
  const assetPerformanceData = useMemo(() => {
    const assetMap: Record<string, { wins: number; losses: number; totalPnl: number; symbolName: string }> = {}
  ...
  const fetchAllData = async () => {
    ...
    // --- CACHED PATH ---
    const cached = globalCache.getAccountData(accountId) as any
    if (cached) {
  ...
  if (isPreLoaded) {
    ...
    const centerContent = isEmptyState ? (
      <>
        <h3 style={{ color: 'var(--foreground)', marginBottom: '12px', fontSize: '20px', fontWeight: '600' }}>Community-Built SoDex Mainnet Scan</h3>
        <p style={{ color: 'var(--muted-foreground)', fontSize: '14px', maxWidth: '500px', lineHeight: '1.6' }}>
      ...
      <>
        <h3 style={{ color: 'var(--foreground)', marginBottom: '12px', fontSize: '20px', fontWeight: '600' }}>Wallet Not Found</h3>
        <p style={{ color: 'var(--muted-foreground)', marginBottom: '24px', fontSize: '14px', maxWidth: '500px', lineHeight: '1.6' }}>
          This wallet address is not currently indexed. Search by SoDex Account ID or another address above.
        </p>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', alignItems: 'center' }}>
          <span style={{ color: 'var(--muted-foreground)', fontSize: '13px' }}>Or scan numerical ID:</span>
      ...
      <>
        <h3 style={{ color: 'var(--foreground)', marginBottom: '12px', fontSize: '20px', fontWeight: '600' }}>Scanning Mainnet Wallet...</h3>
    ...
    return (
      <div className="scanner-grid">
        ...
        <aside className="section-sidebar" style={{ background: 'var(--card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
          {showSkeleton ? (
            <>
              {tagSection}
            ...
            <>
              ...
              <div style={{ padding: '20px', textAlign: 'center' }}>
                <h3 style={{ color: 'var(--foreground)', fontSize: '14px', marginBottom: '8px' }}>Profile Search</h3>
                <p style={{ color: 'var(--muted-foreground)', fontSize: '12px' }}>Enter an Account ID or search for another wallet.</p>
              </div>
            </>
          ) : null}
        </aside>
        ...
        <aside className="section-activity" style={{ background: 'var(--card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: showSkeleton ? '20px' : undefined }}>
          {showSkeleton && (
            <>
    ...
    <div className="scanner-grid">
      {toastMessage && (
        <div style={{
          ...
          <span style={{ fontSize: '12px', fontWeight: '600', color: 'var(--foreground)' }}>{toastMessage}</span>
          <button
      ...
      <aside className="section-sidebar">
        ...
        <div style={{ marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {isEditingAlias ? (
                ...
                <h3 style={{ color: 'var(--foreground)', fontSize: '16px', fontWeight: '700', margin: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', position: 'relative' }}>
            ...
            <button
              ...
              title="Follow this wallet for copy-trade signals"
              style={{
                flex: 1,
                background: 'var(--color-overlay-subtle)',
                border: '1px solid var(--border)',
              ...
              {showEditMenu && (
                ...
                >
                  <button
                    ...
                    style={{ background: 'transparent', border: 'none', color: 'var(--foreground)', padding: '8px 12px', width: '100%', textAlign: 'left', cursor: 'pointer', fontSize: '11px', borderRadius: '4px' }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <span style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Account value</span>
            <h2 style={{ color: 'var(--foreground)', fontSize: '24px', fontWeight: '400', margin: 0 }}>
        ...
        <div style={{ marginBottom: '0' }}>
          <h4 style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Account Equity</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }}>Futures</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }} data-testid="stat-futures-value" aria-label={`Futures Value: ${statsSummary.futuresValue}`}>${formatNumber(statsSummary.futuresValue)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Spot</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }} data-testid="stat-spot-value" aria-label={`Spot Value: ${statsSummary.spotValue}`}>${formatNumber(statsSummary.spotValue)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Vault</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }} data-testid="stat-vault-value" aria-label={`Vault Value: ${statsSummary.vaultValue}`}>${formatNumber(statsSummary.vaultValue)}</span>
            </div>
          </div>
        </div>
        ...
        {/* Section 3: Futures */}
        <div style={{ marginBottom: '0' }}>
          <h4 style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Futures</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Unrealized pnl</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Average Leverage</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }}>{statsSummary.avgLev.toFixed(1)}x</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              ...
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }}>${formatNumber(statsSummary.allTimeVol)}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '0' }}>
          <h4 style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Futures Performance</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Win rate</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }}>{statsSummary.winRate.toFixed(1)}%</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
        ...
        <div style={{ marginBottom: '0' }}>
          <h4 style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Deposit / Withdrawal</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Deposited</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }}>${formatNumber(depositWithdrawalDelta.totalDeposits)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Withdrawn</span>
              <span style={{ color: 'var(--foreground)', fontWeight: '600' }}>${formatNumber(depositWithdrawalDelta.totalWithdrawals)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>Delta</span>
              <span style={{ color: depositWithdrawalDelta.delta > 0 ? '#4ade80' : depositWithdrawalDelta.delta < 0 ? '#f87171' : 'var(--foreground)', fontWeight: '600' }}>
        ...
        {/* Section 5: Rankings */}
        <div style={{ marginBottom: '0' }}>
          <h4 style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Rankings</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
              <span style={{ color: 'var(--muted-foreground)' }}>PnL</span>
        ...
        {/* Section 6: Social */}
        <div style={{ marginBottom: '0' }}>
          <h4 style={{ color: 'var(--foreground)', fontSize: '11px', fontWeight: '700', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Social</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {/* Referral Code */}
            <div
            ...
            >
              <span style={{ color: 'var(--muted-foreground)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                Ref Code
                <span style={{ cursor: 'help', color: 'var(--muted-foreground)', fontSize: '10px' }}>ⓘ</span>
              </span>
              {socialData.ref_code ? (
                <a
                ...
                <span style={{ color: 'var(--muted-foreground)', fontWeight: '600' }}>Unknown</span>
              )}
            ...
            >
              ...
              <span style={{ color: socialData.dc_username ? 'var(--foreground)' : 'var(--muted-foreground)', fontWeight: '600' }}>
            ...
            >
              ...
              <span style={{ color: (socialData.tg_username || socialData.tg_displayname) ? 'var(--foreground)' : 'var(--muted-foreground)', fontWeight: '600' }}>
      ...
      >
        {/* Chart/Calendar toggle — proper header row, no absolute positioning */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          gap: '8px',
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                fontSize: '11px',
                fontWeight: '600',
                background: pnlViewMode === 'chart' ? 'var(--card)' : 'transparent',
                color: pnlViewMode === 'chart' ? 'var(--foreground)' : 'var(--muted-foreground)',
                border: pnlViewMode === 'chart' ? '1px solid var(--border)' : '1px solid transparent',
                borderRadius: '4px',
                cursor: 'pointer',
                transition: 'all 0.15s',
                boxShadow: pnlViewMode === 'chart' ? '0 1px 2px rgba(0,0,0,0.1)' : 'none',
              }}
            >
              Chart
            </button>
            <button
              ...
              style={{
                ...
                fontSize: '11px',
                fontWeight: '600',
                background: pnlViewMode === 'calendar' ? 'var(--card)' : 'transparent',
                color: pnlViewMode === 'calendar' ? 'var(--foreground)' : 'var(--muted-foreground)',
                border: pnlViewMode === 'calendar' ? '1px solid var(--border)' : '1px solid transparent',
                borderRadius: '4px',
                cursor: 'pointer',
                transition: 'all 0.15s',
                boxShadow: pnlViewMode === 'calendar' ? '0 1px 2px rgba(0,0,0,0.1)' : 'none',
              }}
            >
              Calendar
            </button>
        ...
        {pnlHistory.length > 0 ? (
          <>
            <div style={{ height: '100%', display: pnlViewMode === 'chart' ? 'flex' : 'none', flexDirection: 'column' }}>
              <ChartCard
                title=""
                data={pnlHistory}
                series={[
      ...
      <aside className="section-activity" onScroll={handleActivityScroll} style={{ overflowY: 'auto', position: 'relative' }}>
        {isBlockedAccount && (
          <div style={{
            ...
            background: 'var(--card)', opacity: 0.85, zIndex: 10, borderRadius: 'var(--radius)'
          }}>
            <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--muted-foreground)', padding: '8px 16px', background: 'var(--card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
              Account data restricted
            </span>
          </div>
        )}
        <div style={isBlockedAccount ? { filter: 'blur(6px)', pointerEvents: 'none', userSelect: 'none' } : {}}>
        <h3 style={{ color: 'var(--foreground)', fontSize: '15px', marginBottom: '16px', fontWeight: '700', paddingLeft: '0px', marginTop: '4px' }}>Recent Activity</h3>
        <div className="timeline" style={{ paddingLeft: '12px', paddingRight: '12px', marginTop: '4px' }}>
          {(() => {
            ...
            items.map((item, idx) => {
              ...
              const color = item.status === 'pos' ? BULLISH_COLOR :
                item.status === 'neg' ? BEARISH_COLOR :
                  item.status === 'internal' ? 'var(--muted-foreground)' : 'var(--foreground)'
              ...
              return (
                <div key={item.id} className={`timeline-item ${item.status}`} style={{
                  ...
                  <div className="timeline-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', marginBottom: '2px' }}>
                    ...
                    <div className="timeline-content" style={{ color: isTrade ? 'var(--foreground)' : color, fontSize: '11px', fontWeight: '600', paddingLeft: '4px' }}>
                      {isTrade ? (
                    ...
                    <div className="timeline-date" style={{ fontSize: '10px', color: 'var(--muted-foreground)', textAlign: 'right' }}>
                  ...
                  <div className="timeline-details" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: '4px' }}>
                    {isTrade ? (
                      <>
                        <div className="timeline-desc" style={{ fontSize: '10px', color: 'var(--muted-foreground)' }}>
                          {item.size} {coin}
                        </div>
                      ...
                      <>
                        ...
                        <div style={{ fontSize: '10px', fontWeight: '500', color: 'var(--muted-foreground)' }}>
                          Confirmed
                        </div>
                      </>
                    )}
            ...
            <div style={{ color: 'var(--muted-foreground)', fontSize: '12px' }}>No recent activity</div>
          )
          })()}
        </div>
        </div>
      ...
      <div className="section-bottom-center" style={{ position: 'relative' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '0', borderBottom: '1px solid var(--color-border-subtle)', flexShrink: 0 }}>
          <div className="tab-nav-row" style={{
                ...
                {['Futures', 'Spot'].map(mode => (
                  <button
                    ...
                    style={{
                      ...
                      color: balanceView === mode ? THEME_COLORS.primary : 'var(--muted-foreground)',
                      transition: 'all 0.2s ease',
                      margin: '0 1px'
                    }}
                  >
                ...
                {['Perps', 'Spot'].map(mode => (
                  <button
                    ...
                    style={{
                      ...
                      color: tradesView === mode ? THEME_COLORS.primary : 'var(--muted-foreground)',
                      transition: 'all 0.2s ease',
                      margin: '0 1px'
                    }}
                  >
            ...
            {activeTab === 'Trades' && tradesView === 'Perps' && tradeSymbolOptions.length > 0 && (
              <select
                ...
                {['Trade', 'Asset'].map(mode => (
                  <button
                    ...
                    style={{
                      ...
                      color: performanceView === mode ? THEME_COLORS.primary : 'var(--muted-foreground)',
                      transition: 'all 0.2s ease',
                      margin: '0 1px'
                    }}
                  >
        ...
        <div className="tab-scroll-area" onScroll={handleScroll} style={isBlockedAccount ? { filter: 'blur(6px)', pointerEvents: 'none', userSelect: 'none' } : {}}>
            ...
            <div>
              {positions.length === 0 ? (
                <div className="empty-state-container">
                  <p style={{ color: 'var(--muted-foreground)' }}>No open positions</p>
                </div>
              ) : isMobile ? (
                      ...
                      <div key={rowKey} className="mobile-card">
                        <div className="mobile-card-summary" onClick={() => toggleRow('Positions', rowKey)}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <svg className={`mobile-card-arrow ${isExpanded ? 'expanded' : ''}`} width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M3 4.5L6 7.5L9 4.5" stroke="var(--muted-foreground)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                          </div>
                        </div>
                        {isExpanded && (
            ...
            <div>
              ...
              {balanceView === 'Futures' && (
                accountDetails?.balances?.length > 0 ? (
                  ...
                  <div className="empty-state-container">
                    <p style={{ color: 'var(--muted-foreground)' }}>No futures balances found</p>
                  </div>
                )
              )}
              ...
              {balanceView === 'Spot' && (
                spotBalances.length > 0 ? (
                        ...
                        .map((bal, i) => {
                          ...
                          return (
                            <div key={i} className="mobile-card">
                              ...
                              {isExpanded && (() => {
                                const base = getBaseCoin(bal.coin).toUpperCase()
                                const amt = parseFloat(bal.balance) || 0
                                const px = base === 'USDC' ? 1 : (markPrices[base === 'XAUT' ? 'XAUt-USD' : `${base}-USD`] || 0)
                                const usd = amt * px
                                return (
                                  <div className="mobile-card-details">
                                    <div className="mobile-card-detail-cell"><span>Available</span><span>{formatBalance(bal.availableBalance, bal.coin)}</span></div>
                                    ...
                                    <div className="mobile-card-detail-cell"><span>USD Value</span><span style={{ fontWeight: '600' }}>{usd > 0 ? `$${formatNumber(usd)}` : '—'}</span></div>
                                  </div>
                                )
                              })()}
                            </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr>
                          ...
                          <th className={balanceSortField === 'usd' ? 'active' : ''} style={{ textAlign: 'right', cursor: 'pointer' }} onClick={() => handleBalanceSort('usd')}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                              USD Value <SortArrows active={balanceSortField === 'usd'} dir={balanceSortDir} />
                            </div>
                          </th>
                        </tr>
                      </thead>
                        ...
                        {[...spotBalances]
                          ...
                          .sort((a, b) => {
                            ...
                            switch (balanceSortField) {
                              ...
                              case 'available': valA = parseFloat(a.available); valB = parseFloat(b.available); break
                              case 'frozen': valA = parseFloat(a.frozen); valB = parseFloat(b.frozen); break
                              case 'usd': valA = a._usd; valB = b._usd; break
                              default: return 0
                            }
                            if (valA < valB) return -1 * dir
                            if (valA > valB) return 1 * dir
                          ...
                          .map((bal, i) => (
                            <tr key={i}>
                              ...
                              <td style={{ textAlign: 'right', fontWeight: '600' }}>
                                {bal._usd > 0 ? `$${formatNumber(bal._usd)}` : <span style={{ color: 'var(--muted-foreground)' }}>—</span>}
                              </td>
                            </tr>
                          ))}
                      </tbody>
                  ...
                  <div className="empty-state-container">
                    <p style={{ color: 'var(--muted-foreground)' }}>No spot balances found</p>
                  </div>
                )
              )}
            </div>
          ...
          {activeTab === 'Orders' && (
            <div className="empty-state-container">
              <p style={{ color: 'var(--muted-foreground)' }}>No active orders found.</p>
            </div>
          )}
          ...
          {activeTab === 'Trades' && tradesView === 'Perps' && (
            <div>
              {positionHistory.length === 0 ? (
                <div className="empty-state-container">
                  <p style={{ color: 'var(--muted-foreground)' }}>No position history found</p>
                </div>
              ) : isMobile ? (
                <div className="mobile-card-list">
                      ...
                      <div key={i} className="mobile-card">
                        ...
                        {isExpanded && (
                          <div className="mobile-card-details">
                            ...
                            <div className="mobile-card-detail-cell"><span>Dates</span><span style={{ color: 'var(--muted-foreground)' }}>{new Date(pos.created_at).toLocaleDateString()} - {new Date(pos.updated_at).toLocaleDateString()}</span></div>
                          </div>
                        )}
                      </div>
                    )
                      ...
                      {sortedTrades.map((pos, i) => {
                          ...
                          return (
                            <tr key={i}>
                              ...
                              <td style={{ textAlign: 'right', color: 'var(--muted-foreground)', fontSize: '10px' }}>
          ...
          {activeTab === 'Transfers' && (
            <div>
              {withdrawals.length === 0 && fundTransfers.length === 0 ? (
                <div className="empty-state-container">
                  <p style={{ color: 'var(--muted-foreground)' }}>No withdrawals or transfers found</p>
                </div>
              ) : isMobile ? (
                <div className="mobile-card-list">
                  {sortedWithdrawals.map((w, i) => {
                    ...
                    const flowColor = isDeposit ? BULLISH_COLOR : isWithdraw ? BEARISH_COLOR : isInternal ? INTERNAL_COLOR : 'var(--foreground)'
                    ...
                    return (
                      <div key={i} className="mobile-card">
                        ...
                        {isExpanded && (
                          <div className="mobile-card-details">
                            <div className="mobile-card-detail-cell"><span>Time</span><span style={{ color: 'var(--muted-foreground)' }}>{formatTimeShort(w.stmp)}</span></div>
                        ...
                        {sortedWithdrawals.map((w, i) => {
                            ...
                            return (
                              <tr key={i}>
                                ...
                                <td style={{ color: isSpecialLabel(fromAddr) ? 'var(--foreground)' : 'inherit' }}>
                                  {isSpecialLabel(fromAddr) ? fromAddr : <CopyableAddress address={fromAddr} />}
                                </td>
                                <td style={{ color: isSpecialLabel(toAddr) ? 'var(--foreground)' : 'inherit' }}>
          ...
          {activeTab === 'Funding' && (
            <div>
              {fundingHistory.length === 0 ? (
                <div className="empty-state-container">
                  <p style={{ color: 'var(--muted-foreground)' }}>No funding history found</p>
                </div>
              ) : isMobile ? (
                <div className="mobile-card-list">
                      ...
                      <div key={i} className="mobile-card">
                        <div className="mobile-card-summary">
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1, minWidth: 0 }}>
                            ...
                            {f.positionSide && <span style={{ fontSize: '9px', color: 'var(--muted-foreground)', fontWeight: 500 }}>{f.positionSide}</span>}
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {fundingHistory.map((f, i) => {
                        ...
                        return (
                          <tr key={i}>
                            <td style={{ color: 'var(--muted-foreground)', fontSize: '11px' }}>
                              {ts ? formatTimeShort(typeof ts === 'string' ? Date.parse(ts) / 1000 : ts) : '-'}
                            </td>
                            <td style={{ fontWeight: '600' }}>{f.symbol || '-'} {f.positionSide && <span style={{ fontSize: '9px', color: 'var(--muted-foreground)', fontWeight: 500 }}>{f.positionSide}</span>}</td>
          ...
          {activeTab === 'Trades' && tradesView === 'Spot' && (
            <div>
              {spotTradeHistory.length === 0 ? (
                <div className="empty-state-container">
                  <p style={{ color: 'var(--muted-foreground)' }}>No spot trades found</p>
                </div>
              ) : isMobile ? (
                <div className="mobile-card-list">
          ...
          {activeTab === 'Performance' && (
            <div>
              {positionHistory.length === 0 ? (
                <div className="empty-state-container">
                  <p style={{ color: 'var(--muted-foreground)' }}>No trade history available</p>
                </div>
                              ...
                              <tr key={i}>
                                ...
                                <td style={{ textAlign: 'center' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '80px', fontSize: '9px', color: 'var(--muted-foreground)' }}>
      ...
      {showCopyModal && walletAddress && (
        <FollowModal
          leaderWallet={walletAddress}
          onClose={() => setShowCopyModal(false)}
          onSuccess={() => { setShowCopyModal(false); showToast('Copy signals enabled') }}
        />
      )}
    </div >
  )
