...
import { ACCENT_PRESETS, COLOR_SCHEMES, isValidHex } from '../lib/themes'
...
function ThemeCustomizer() {
  const {
    mode, colorScheme, settings, primaryColor,
    setMode, setColorScheme, setCustomPrimary, resetCustomPrimary,
  } = useTheme()

  const effectivePrimary = settings.customPrimary && isValidHex(settings.customPrimary)
    ? settings.customPrimary
    : primaryColor || (COLOR_SCHEMES[colorScheme]?.[mode]?.primary ?? '#f26b1f')

  const [hex, setHex] = useState(effectivePrimary)
  useEffect(() => { setHex(effectivePrimary) }, [effectivePrimary])

  const onHexChange = (val: string) => {
    setHex(val)
    if (isValidHex(val)) setCustomPrimary(val)
  }

  const schemeOptions = Object.entries(COLOR_SCHEMES).map(([id, def]) => ({
    id: id as keyof typeof COLOR_SCHEMES,
    name: def.name,
    primary: def[mode]?.primary ?? def.dark.primary,
  }))

  const isCustom = !!(settings.customPrimary && isValidHex(settings.customPrimary))

  return (
    <div className="ds-theme-customizer">
      <div className="ds-tc-row">
        <span className="ds-tc-label">Mode</span>
        <div className="ds-tc-mode-toggle" role="radiogroup" aria-label="Theme mode">
          <button
            type="button"
            role="radio"
            aria-checked={mode === 'dark'}
            className={`ds-tc-mode-btn ${mode === 'dark' ? 'active' : ''}`}
          ...
          <button
            ...
            className={`ds-tc-mode-btn ${mode === 'light' ? 'active' : ''}`}
      ...
      <div className="ds-tc-row">
        <span className="ds-tc-label">
          Primary color
          {isCustom && <span className="ds-tc-badge">Custom</span>}
        </span>
        <div className="ds-tc-color-controls">
          <label className="ds-tc-swatch-input" title="Pick a color">
            <span className="ds-tc-swatch-preview" style={{ background: effectivePrimary }} />
            <input type="color" value={effectivePrimary} onChange={(e) => onHexChange(e.target.value)} />
          </label>
          <input
            type="text"
            className="ds-tc-hex-input"
            value={hex}
          ...
          <button
            type="button"
            className="ds-tc-reset-btn"
            onClick={resetCustomPrimary}
            disabled={!isCustom}
            title="Use the active scheme's default primary"
          >
            Reset
          </button>
        </div>
        <div className="ds-tc-presets">
          {ACCENT_PRESETS.map(c => (
            <button
              key={c}
              type="button"
              className={`ds-tc-preset ${effectivePrimary?.toLowerCase() === c.toLowerCase() ? 'active' : ''}`}
              style={{ background: c }}
      ...
      <div className="ds-tc-row">
        <span className="ds-tc-label">Color scheme</span>
        <div className="ds-tc-schemes">
          {schemeOptions.map(s => (
            <button
              key={s.id}
              type="button"
              className={`ds-tc-scheme ${colorScheme === s.id ? 'active' : ''}`}
              onClick={() => setColorScheme(s.id)}
              title={s.name}
            >
              <span className="ds-tc-scheme-dot" style={{ background: s.primary }} />
              <span className="ds-tc-scheme-name">{s.name}</span>
            </button>
          ))}
        </div>
        <p className="ds-tc-hint">
          Schemes set background, text, and border tokens. Your picked primary overrides the scheme&apos;s accent across the whole app.
        </p>
      </div>
    </div>
  )
...
function PixelChartDemo() {
  ...
  return (
    <div className="ds-pixel-demo">
      <div className="ds-pixel-grid" style={{ '--cols': COLS, '--rows': ROWS } as React.CSSProperties}>
        {cols.map((col, ci) => (
          <div
            key={ci}
    ...
    <div className="ds-page">
      ...
      {tab === 'tokens' && (
        <TabWithSidebar sections={TOKEN_SECTIONS}>
          <Canvas id="ds-theme" title="Theme" desc="Pick the primary color and mode for the entire app. Changes apply instantly across every page and persist in this browser.">
            <ThemeCustomizer />
          </Canvas>
          <Canvas id="ds-colors" title="Color Tokens" desc="Click any chip to copy the value. The accent group reflects the picked primary color above.">
            <ColorPalette />
          </Canvas>
