...
export default function PerformanceWidget({ config, onUpdateConfig }) {
  ...
  type ClosedTrade = { realized_pnl?: string | number; symbol?: string; coin?: string; pair?: string; instrument_name?: string; pnl?: string | number }
  ...
  const tradeStats = useMemo(() => {
    ...
    const pnls = closedTrades.map(t => parseFloat(String(t.realized_pnl || 0)))
  ...
  const assetStats = useMemo(() => {
    ...
    const bySymbol: Record<string, { symbol: string; pnl: number; trades: number; wins: number }> = {}
    closedTrades.forEach(t => {
      ...
      const pnl = parseFloat(String(t.realized_pnl || 0))
