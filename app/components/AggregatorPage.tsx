  ...
  return (
    ...
    <div className={`agg-root ${navDockClass}${agg.performanceMode ? ' perf-mode' : ''}`}>
      ...
      <div className={`agg-content${devicePreview ? ' agg-content--device-preview' : ''}`}>
        {/* Sub-navigation/heading bar */}
        <div className="agg-subnav">
          <div className="agg-subnav-left">
            <h2 className="agg-subnav-title">{agg.activePage?.name || 'Aggregator'}</h2>
            {currentPageLocked && (
              <span className="agg-subnav-pill" title="This page is locked">Locked</span>
            )}
            {agg.performanceMode && (
              <span className="agg-subnav-pill agg-subnav-pill--accent" title="Performance mode">Perf</span>
            )}
          </div>
          <div className="agg-subnav-right">
            {!currentPageLocked && (
              <button
                type="button"
                className="agg-subnav-btn"
            ...
            <button
              type="button"
              className={`agg-subnav-btn${(agg.editMode !== false && !currentPageLocked) ? ' is-active' : ''}`}
              onClick={() => { if (!currentPageLocked) agg.setEditMode(!(agg.editMode !== false)) }}
            ...
            <button
              type="button"
              className={`agg-subnav-btn${showAssistant ? ' is-active' : ''}`}
