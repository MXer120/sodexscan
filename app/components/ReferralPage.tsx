...
function SocialBadge({ platform, handle, displayName, refCode, link }: SocialBadgeProps) {
  ...
  const badgeClassName = `social-badge ${isUnknown ? 'unknown' : ''}`
...
// Featured referral code (always on top)
const FEATURED_REF_CODE = 'SOSO'
...
function ReferralCard({ referral, featured }: ReferralCardProps) {
  const link = `${BASE_URL}${referral.ref_code}`

  return (
    <div className={`referral-card ${featured ? 'referral-card-featured' : ''}`}>
      ...
      <div className="social-badges">
        ...
        <SocialBadge
          platform="X"
          handle={referral.ref_code === 'SOSO' ? 'Lutz_S120' : null}
          refCode={referral.ref_code}
          link={referral.ref_code === 'SOSO' ? 'https://x.com/Lutz_S120' : undefined}
        />
      </div>
    </div>
  )
...
export default function ReferralPage() {
  ...
  const filteredReferrals = useMemo(() => {
    // Find featured referral
    const featuredRef = referrals.find(ref => ref.ref_code === FEATURED_REF_CODE)
    const otherRefs = referrals.filter(ref => ref.ref_code !== FEATURED_REF_CODE)

    if (!debouncedSearch.trim()) {
      // No search: featured first, then others
      return featuredRef ? [featuredRef, ...otherRefs] : otherRefs
    }

    const term = debouncedSearch.toLowerCase()
    const matchingOthers = otherRefs.filter(ref =>
    ...
    // Always include featured at top, even when searching
            ...
            <div className="referral-grid">
              {filteredReferrals.map((ref) => (
                <ReferralCard
                  key={ref.id}
                  referral={ref}
                  featured={ref.ref_code === FEATURED_REF_CODE}
                />
              ))}
            </div>
