...
const getPnlBgColor = (pnl, minPnl, maxPnl, isBestDay) => {
  ...
  if (pnl >= 0.01) {
    ...
    // Subtle bearish color - max alpha 0.25
      ...
      days.push(
        <div key={i}
          ...
          style={{
            ...
            border: isBestDay ? `1px solid ${PRIMARY_COLOR}` : '1px solid var(--border)',
          ...
          <div style={{ fontSize: 'clamp(7px, 1.2vw, 9px)', color: isBestDay ? PRIMARY_COLOR : 'var(--muted-foreground)', lineHeight: 1 }}>
            {dayNames[i]}
          </div>
          <div style={{ fontSize: 'clamp(8px, 1.4vw, 11px)', color: isBestDay ? PRIMARY_COLOR : 'var(--muted-foreground)', lineHeight: 1.2, marginTop: '2px' }}>
            {date.getDate()}
          </div>
          <div style={{
            flex: 1,
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      cells.push(
        <div key={d}
          ...
          <div style={{ fontSize: 'clamp(7px, 1.2vw, 9px)', color: isBestDay ? PRIMARY_COLOR : 'var(--muted-foreground)', lineHeight: 1 }}>{d}</div>
          <div style={{
            flex: 1,
            display: 'flex',
            alignItems: 'center',
      ...
      <div style={{
        ...
        <div style={{
          ...
          {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
            <div key={i} style={{ textAlign: 'center', fontSize: 'clamp(7px, 1.2vw, 10px)', color: 'var(--muted-foreground)' }}>{d}</div>
          ))}
        </div>
        {/* Calendar grid */}
        <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, width: '100%', minWidth: 0, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto', overflowY: 'hidden', paddingBottom: '4px' }}>
          <div style={{
            ...
            fontSize: 'clamp(7px, 1.2vw, 10px)', color: 'var(--muted-foreground)',
          ...
          <div style={{ display: 'flex', gap: `${cellGap}px`, minWidth: 'max-content' }}>
            <div style={{
              ...
              fontSize: 'clamp(6px, 1vw, 8px)', color: 'var(--muted-foreground)',
              flexShrink: 0,
            }}>
    ...
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <button
            ...
            style={{
              ...
              fontSize: '10px',
              outline: 'none',
              WebkitTapHighlightColor: 'rgba(72, 203, 255, 0.3)'
            }}
          >
            ←
          </button>
          <span style={{ fontSize: '10px', color: 'var(--muted-foreground)', minWidth: '80px', textAlign: 'center' }}>
            {getPeriodLabel()}
          </span>
          <button
            onClick={() => navigate(1)}
          ...
          {[{ key: 'rolling', label: '1Y' }, { key: 'weekly', label: 'W' }, { key: 'monthly', label: 'M' }, { key: 'yearly', label: 'Y' }].map(v => (
            <button
              ...
              style={{
                ...
                color: view === v.key ? PRIMARY_COLOR : 'var(--muted-foreground)',
                border: 'none',
                borderRadius: '3px',
                cursor: 'pointer',
                transition: 'all 0.2s',
      ...
      {hoveredDay && typeof document !== 'undefined' && createPortal(
        <div style={{
          ...
          <div style={{ fontSize: '10px', color: 'var(--muted-foreground)', fontWeight: '500', marginBottom: '2px' }}>
      ...
      <div
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: '18px', color: 'var(--foreground)', fontWeight: '700' }}>
              {new Date(selectedDate + 'T00:00:00Z').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric', timeZone: 'UTC' })}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '11px', color: 'var(--muted-foreground)', marginBottom: '2px' }}>Net PnL</div>
        ...
        <div style={{ padding: '20px' }}>
          <div style={{ fontSize: '12px', color: 'var(--muted-foreground)', marginBottom: '12px', fontWeight: '600', textTransform: 'uppercase' }}>
            ...
            {tradesByDate[selectedDate]?.length > 0 ? (
              <>
                    ...
                    <div key={idx} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ fontWeight: '700', color: 'var(--foreground)', fontSize: '14px' }}>{sym}</span>
                          <span style={{
                            fontSize: '10px',
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                        <div>
                          <div style={{ fontSize: '10px', color: 'var(--muted-foreground)', textTransform: 'uppercase', marginBottom: '2px' }}>Entry</div>
                          <div style={{ fontSize: '12px', color: 'var(--foreground)', fontWeight: '500' }}>${parseFloat(trade.avg_entry_price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '10px', color: 'var(--muted-foreground)', textTransform: 'uppercase', marginBottom: '2px' }}>Exit</div>
                          <div style={{ fontSize: '12px', color: 'var(--foreground)', fontWeight: '500' }}>${parseFloat(trade.avg_close_price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '10px', color: 'var(--muted-foreground)', textTransform: 'uppercase', marginBottom: '2px' }}>Size</div>
                          <div style={{ fontSize: '12px', color: 'var(--foreground)', fontWeight: '500' }}>{trade.max_size}</div>
                        </div>
                      </div>
                    </div>
                  )
                ...
                {tradesByDate[selectedDate].length > 3 && (
                  <div style={{ textAlign: 'center', padding: '10px', fontSize: '12px', color: 'var(--muted-foreground)' }}>
                    + {tradesByDate[selectedDate].length - 3} more trades. View trade history for more info.
                  </div>
                )}
              </>
            ) : (
              <div style={{ textAlign: 'center', padding: '40px 0', border: '1px dashed var(--border)', borderRadius: '10px' }}>
                <div style={{ color: 'var(--muted-foreground)', fontSize: '12px' }}>No trades opened on this day</div>
              </div>
            )}
          </div>
        </div>
